 ...
 *
 * --- Cocovox / corey local tweak (krunner-row-tweak) -------------------------
 * Bold result rows: 48px icons, filename + always-visible path on a second
 * line, roomier vertical padding. Re-applied after milou upgrades by the
 * pacman hook /etc/pacman.d/hooks/krunner-row-tweak.hook. Original backed up at
 * /usr/local/share/krunner-row-tweak/ResultDelegate.qml.orig.
 * ----------------------------------------------------------------------------
 */
...
PlasmaComponents3.ItemDelegate {
    ...
    implicitHeight: labelWrapper.implicitHeight + Kirigami.Units.largeSpacing * 2
    ...
    property int categoryWidth: Kirigami.Units.gridUnit * 6
        ...
        RowLayout {
            ...
            ColumnLayout {
                ...
                PlasmaComponents3.Label {
                    id: subtextLabel
                    Layout.fillWidth: true
                    // Always show the secondary line (path / metadata) when the
                    // runner provides one — not only for the selected row.
