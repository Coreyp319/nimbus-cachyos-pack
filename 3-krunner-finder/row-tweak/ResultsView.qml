...
ListView {
    ...
    // krunner-row-tweak: glide the selection highlight instead of snapping
    highlightMoveVelocity: -1
    highlightMoveDuration: 130
    activeFocusOnTab: true
    Accessible.role: Accessible.List

    // --- krunner-row-tweak: animate rows in as they are inserted ----------
    // Fires when results are added to the model. Late-arriving rows (e.g. the
    // debounced "Ask Claude" / "Search the web" entries) fade + slide in;
    // neighbours glide to make room via the displaced transition.
    add: Transition {
        NumberAnimation { property: "opacity"; from: 0.0; to: 1.0; duration: 220; easing.type: Easing.OutCubic }
        NumberAnimation { property: "y"; from: ViewTransition.destination.y + 8; duration: 220; easing.type: Easing.OutCubic }
    }
    ...
    remove: Transition {
        NumberAnimation { property: "opacity"; to: 0.0; duration: 160; easing.type: Easing.InCubic }
        NumberAnimation { property: "y"; to: ViewTransition.item.y + 8; duration: 160; easing.type: Easing.InCubic }
    }
    // ---------------------------------------------------------------------
